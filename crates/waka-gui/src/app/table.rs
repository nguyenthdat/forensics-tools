...
impl TableEditor {
    ...
    fn export_current_to_json_path(&self, dest: &Path, only_filtered: bool) -> anyhow::Result<()> {
        let Some(fp) = self.current_fp() else {
    ...
    fn export_current_to_csv_path(&self, dest: &Path, only_filtered: bool) -> anyhow::Result<()> {
        let Some(fp) = self.current_fp() else {
