...
impl SelectColumns {
    pub fn parse(mut s: &str) -> anyhow::Result<SelectColumns> {
...
impl SelectorParser {
    ...
    fn parse(&mut self) -> anyhow::Result<Vec<Selector>> {
        if (self.chars.first(), self.chars.last()) == (Some(&'/'), Some(&'/')) {
            ...
            let Ok(regex) = Regex::new(&re) else {
                return Err(anyhow!("Invalid regex: {re}"));
            };
    ...
    fn parse_one(&mut self) -> anyhow::Result<OneSelector> {
    ...
    fn parse_quoted_name(&mut self) -> anyhow::Result<String> {
        ...
        loop {
            match self.cur() {
                None => {
                    return Err(anyhow!("Unclosed quote, missing closing \"."));
                },
                Some('"') => {
                    self.bump();
                    if self.cur() == Some('"') {
    ...
    fn parse_index(&mut self) -> anyhow::Result<usize> {
        ...
        loop {
            match self.cur() {
                None => {
                    return Err(anyhow!("Unclosed index bracket, missing closing ]."));
                },
                Some(']') => {
                    self.bump();
                    break;
        ...
        FromStr::from_str(&idx)
            .map_err(|err| anyhow!("Could not convert '{idx}' to an integer: {err}"))
    }
...
impl OneSelector {
    fn index(&self, first_record: &csv::ByteRecord, use_names: bool) -> anyhow::Result<usize> {
        match *self {
