...
impl Delimiter {
    ...
    pub fn decode_delimiter(s: &str) -> anyhow::Result<Delimiter> {
        if s == r"\t" {
        ...
        if c.is_ascii() {
            ...
            Err(anyhow!("Could not convert '{c}' to ASCII delimiter."))
        }
    }
}
...
impl Config {
    ...
    pub fn selection(&self, first_record: &csv::ByteRecord) -> anyhow::Result<Selection> {
        match self.select_columns {
            None => Err(anyhow!(
                "Config has no 'SelectColumns'. Did you call Config::select?"
            )),
    ...
    #[inline]
    pub fn indexed(&self) -> anyhow::Result<Option<Indexed<fs::File, fs::File>>> {
