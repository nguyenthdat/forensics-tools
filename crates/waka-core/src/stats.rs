...
fn parse_boolean_patterns(boolean_patterns: &str) -> anyhow::Result<Vec<BooleanPattern>> {
    ...
    for pair in boolean_patterns.split(',') {
        ...
        if true_pattern.is_empty() || false_pattern.is_empty() {
            return Err(anyhow!("Invalid boolean pattern: {pair}"));
        }
    ...
    if patterns.is_empty() {
        return Err(anyhow!("Boolean patterns must have at least one pattern"));
    }
    Ok(patterns)
}
...
pub fn run(argv: &[&str]) -> anyhow::Result<()> {
    ...
    if rconfig.is_stdin() {
        ...
        let (mut preview_file, tempfile_path) = stdin_file
            .keep()
            .or(Err(anyhow!("Cannot keep temporary file")))?;
    ...
    if let Some(path) = rconfig.path.clone() {
        ...
        if compute_stats {
            ...
            // tracing::info!("scanned {record_count} records...");
            ...
            if args.flag_dataset_stats {
                ...
                let mut write_dataset_stat = |name: &[u8], value: u64| -> anyhow::Result<()> {
    ...
    if rconfig.is_stdin() {
            ...
            || (args.flag_cache_threshold.is_negative() && args.flag_cache_threshold % 10 == -5)
        {
            ...
            if autoindex_set {
                ...
                tracing::debug!("deleting index file: {}", index_file.display());
                if std::fs::remove_file(index_file.clone()).is_err() {
                    // fails silently if it can't remove the index file
                    tracing::warn!("Could not remove index file: {}", index_file.display());
                }
            }
...
impl Args {
    ...
    fn sequential_stats(&self, whitelist: &str) -> anyhow::Result<(csv::ByteRecord, Vec<Stats>)> {
    ...
    fn parallel_stats(
        ...
        whitelist: &str,
        idx_count: u64,
    ) -> anyhow::Result<(csv::ByteRecord, Vec<Stats>)> {
    ...
    ) -> anyhow::Result<(csv::ByteRecord, Selection)> {
...
fn init_date_inference(
    ...
    if !infer_dates {
        ...
        INFER_DATE_FLAGS
            ...
            .map_err(|_| anyhow!("Cannot init empty date inference flags"))?;
        return Ok(());
    }

    let infer_date_flags = if flag_whitelist.eq_ignore_ascii_case("all") {
        tracing::info!("inferring dates for ALL fields");
        SmallVec::from_elem(true, headers.len())
    } else {
        let mut header_str = String::new();
        let whitelist_lower = flag_whitelist.to_lowercase();
        tracing::info!("inferring dates with date-whitelist: {whitelist_lower}");
    ...
    INFER_DATE_FLAGS
        ...
        .map_err(|e| anyhow!("Cannot init date inference flags: {e:?}"))?;
    Ok(())
}
