...
impl<R: io::Read + io::Seek, W: io::Write> IoState<R, W> {
    ...
    fn exclude(mut self, invert: bool) -> anyhow::Result<()> {
...
impl Args {
    fn new_io_state(&self) -> anyhow::Result<IoState<fs::File, Box<dyn io::Write + 'static>>> {
        let rconf1 = Config::builder()
            .path(&self.arg_input1)
            .build()
            .delimiter(self.flag_delimiter)
            .no_headers(self.flag_no_headers)
            .select(self.arg_columns1.clone());
        let rconf2 = Config::builder()
            .path(&self.arg_input2)
            .build()
            .delimiter(self.flag_delimiter)
            .no_headers(self.flag_no_headers)
        ...
        Ok(IoState {
            wtr: Config::builder()
                .maybe_path(self.flag_output.as_ref())
                .build()
                .writer()?,
            rdr1,
            sel1,
    ...
    ) -> anyhow::Result<(Selection, Selection)> {
...
impl<R: io::Read + io::Seek> ValueIndex<R> {
    fn new(mut rdr: csv::Reader<R>, sel: &Selection, casei: bool) -> anyhow::Result<ValueIndex<R>> {
